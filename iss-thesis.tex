...
\gls{rgb}-only keypoint matching is the most common method, because the \gls{2d} data is more accessable than the data with additional depth information and the networks dealing with \gls{2d} image are more mature as well. Besides the hand-crafted feature discriptor that we introduced before, methods like \cite{pavlakos20176dof, rad2018bb8, pmlr-v87-tremblay18a,tekin2018realtime, zakharov2019dpod} use the \gls{cnn}-based architecture to extract the \gls{2d} semantic keypoints and estimate the pose by \gls{pnp} algorithm. Especially, Oberweger et al. 2018 \cite{oberweger2018making} propose a \gls{rgb}-only method that can estimate the pose under large occlussion. Their solution is to predict heatmaps from multiple patches independently and calculate the accumulated score to ensure the robust estimation. ZebraPose \cite{su2022zebrapose} assigns a hierarchical binary grouping to encode the object surface. It replaces the sparse correspondance matching with dense maps learning to handle the occluded object and make the learning of \gls{2d}-\gls{3d} correspondances more efficient.
...
Typical work from Hsiao et al. 2023 \cite{hsiao2023confronting} focus on the advantages of diffusion models towards the pose ambiguity problem on $\mathbb{SE}(3)$. The denoiser backbone is consist of several \gls{mlp} blocks processing the \gls{2d} global feature extracted from a pretrained ResNet. Figure \ref{img:confront} shows the overview of the framework. The Authers use the surrogate Stein score calculation on $\mathbb{SE}(3)$ following the score-based diffusion architecture and Fourier-based conditioning mechanism to deal with the ambiguity problem caused by the symetrical object. Additionally, they also compare the advantage of $\mathbb{SE}(3)$ parametrization over previous works on $R^{3}\mathbb{SE}(3)$. This work is trained and evaluated on the synthetic dataset SYMSOL \cite{implicitpdf2021} with fice texture-less objects, namely tetrahedron, cube, icosahedron, cone, and cylinder. The denoising process is shown in figure \ref{img:confront_cube}. The real data captured by the camera is not considered in this work. In our proposed method, we focus on the pose estimation with diffusion models under the real-world scenario and even with occlusion, which is more general and challenging.
\begin{figure}[h]
	\centering
...
By generating multiply samples for one reference input $y$, we can get a set of pose hypotheses $\mathbf{T}_{1},...,\mathbf{T}_{N}$, which compose a hypothesis distribution $h(\mathbf{T}|y)$ that can be used to estimate the pose $\mathbf{T}$ of the reference input $\mathbf{r}$. For a given object without any ambiguity, the distribution $h(\mathbf{T}|y)$ should be a delta distribution in ideal case, or in other words, squeezed to a single point in the spatial solving space. The pose hypotheses $\mathbf{T}_{1},...,\mathbf{T}_{N}$ should be close to each other and the variance of the distribution should be small. On the contrary, if the object is symmetrical, the distribution of the pose should fit the corresponding pattern of the symmetry in the solving space. In our work, we focus on the first case that we don't propose any post-processing method to analyize the distribution of the symetrical object, which can be a future work.
...
One of the advantages of diffusion model is the high quality and diversity of the generated objects. Using the case of image synthesis as an example, we can derive dozen of high-resolution and meaningful images from the diffusion model with the same prompt. In our case, we want to first use the multi-hypotheses module to reduce the variance of the generated poses by taking the mean of all hypotheses as the final estimation and make the final result more robust. In the feature work, we may sample more hypotheses to analyize the pose distribution of symetrical objects in order to explore more possibility when we dealing with the ambiguity of the pose estimation.
...
The following figure \ref{img:fold_dev} shows the development of the output from FoldingNet decoder during the training process.
...
From the visualization we can find that the shape of the reconstructed point cloud is getting more and more similar to the ground truth, but the difference exists because of the capability of the backbone on data with heavy occlusion and noise. We further compare the training of point cloud completion task on all 15 categories with overfitted models on single object to validate the generalization of the network. From figure \ref{img:fold_overfit} we can find that the \gls{cd} of the overfitted the models has lower mean and variance than the model trained on all categories. Table \ref{tab:fold_overfit} shows the quantitative comparision.
\begin{figure}[h]
	\centering
	\includegraphics[width=1.\textwidth]{img/fold_overfit.eps}
	\caption{Comparision of the CD distributions between the model trained with the whole data set and the overfitted model on single object (the first 7 objects of LM are shown)}
	\label{img:fold_overfit}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Mean and standard deviation of the CD with the model trained on the whole dataset and the overfitted model on single object (7 of 15 objects selected)}
...
We evaluate our model on the test dataset of \gls{bop} challenge \cite{hodan2018bop} with overall 3000 images and use the segmentation result from the ground truth as our mask to filter out the background, since we only focus on the pose estimation with object already detected. In this section, we represent both quantitative results and qualitative estimation of given input. Our method is also compared with the state-of-the-art methods on this task.

\subsubsection{Quantitative Evaluation}
Following the metrics that we introduced in previous section, we propose comprehensive evaluations on each object of both \gls{lm} and \gls{lmo} datasets. The corresponding results of all categories are shown in table \ref{tab:eval_categories_lm} and \ref{tab:eval_categories_lmo}. In figure \ref{img:eval_threshold} we can further validate the capability of our model facing each objects with different threshold of correctness. Object like cup, whose \gls{avre} of \gls{bop} metrics and \gls{add}(-S) is relative lower, is hard to estimate. This is because the ambiguity cause by the handle when it is not visible and this identification part only takes a small partition in the whole object. Besides that, our model achieves quiet satisfying result even dealing with the object with occlusion.
\begin{table}[h]
  \centering
  \caption{Quantitative Evaluation on LM dataset(symetrical objects annotated with $^{*}$)}
...
\begin{table}[h]
  \centering
  \caption{Quantitative Evaluation on LMO dataset(symetrical objects annotated with $^{*}$)}
...
Comparing with other \gls{6dof} pose estimation methods in the recent years (table \ref{tab:eval_compare_lm}, \ref{tab:eval_compare_lmo}) including \gls{rgb}-only and \gls{rgbd} approaches, our model owns comparable performance especially on \gls{lmo} dataset. It is easy to notice the significant \gls{avre} drop between the result on \gls{lm} and \gls{lmo} of some methods, which is even more than $40\%$. Our method however has an acceptable score with and without refinement. Why we only use \gls{add}(-S) as the evaluation metric in this part of comparsion, is because the majority of pose estimation methods only provide \gls{add}(-S) in publications and the result of \gls{bop} metrics are not always available. Our ablation study based on all the metrics of \gls{bop} is shown in the next section.

\begin{table}[h]
  \centering
  \caption{Comparison of ADD(-S) scores with other methods on LM dataset (symetrical objects annotated with $^{*}$)}
...
\begin{table}[h]
  \centering
  \caption{Comparison of ADD(-S) scores with other methods on LMO dataset (symetrical objects annotated with $^{*}$)}
...
It is worth to mention that our model is not trained on each objects seperately, so the result can be further improved if we train the model overfitted to each object with the cost of unflexibility. Table \ref{tab:eval_overfit_lmo} shows the difference of the \gls{avre} with a generalized model and object-specific one evaluating on \gls{lmo}.
\begin{table}[h]
  \centering
...
Here we also demonstrate the qualitative result of our model on the pose estimation task by using the visualisation toolkit from BOP \cite{hodan2018bop} challenge. Figure \ref{img:eval_vis_img} and \ref{img:vis_all} show some objects from \gls{lm} and \gls{lmo} dataset rendered with the estimated pose on the input image. First row is the objects without occlusion and second row is the objects with occlusion. We can observe that the \gls{2d}-projection of the estimated pose has a very high percentage of overlap with the ground truth pose if there is no occlusion. The highlighted estimation and object in the background is shifted a little bit when the visible part of the object is occluded by other objects.
\begin{figure}[h]
	\centering
...
Additionally, we can have a closer look at the development of the pose hypotheses through the whole denoising process (figure \ref{img:eval_vis_denoise}). We select the cat object in \gls{lm} dataset as an example and render the visualization with Blender. The multi-hypotheses (set to 16 in our experiment) of estimated pose, which is colorized with transparent gray, localize randomly in the space at the early stage of the denoising process and finally overlap together on the ground truth (purple). More examples of the visualization can be found in the appendix \ref{img:vis_cat}, \ref{img:vis_bench}.  
\begin{figure}[h]
	\centering
...
Recall the discussion in section \ref{sec:representation} and \ref{sec:res_t}, we compare the different representation of rotation including quaternion, \gls{mrp} and 6D representation \cite{Zhou_2019_CVPR}. From table \ref{tab:ab_rot_lm} and \ref{tab:ab_rot_lmo} we can find that the 6D representation and quaternion have equally best performance on \gls{lm} dataset, and 6D achieves better result on the \gls{lmo} without refinement. Figure \ref{img:ab_rot} shows the convergance of the rotation error with three different forms. We can also see the difference between each representation.
...
Table \ref{tab:ab_trans_lm} and \ref{tab:ab_trans_lmo} show the results of the model trained with and without residual translation. The advantage of learning the residual error is clear that the \glspl{ar} are improved in both datasets. We can benefit from this trick with respect to the convergance speed shown in figure \ref{img:ab_trans}. The residual translation is also a good way to avoid the model to be trapped in the local minimum.
\begin{table}[h]
  \centering
...
In the orignial \gls{ddpm} setting of image synthesis task, the diffusion steps is configured as 1000 to make sure the vividness of the generated image. In our pose estimation task, we compare the effects caused by the number of denoising steps on the final result in table \ref{tab:ab_steps}. The result shows that large number of steps doesn't necessarily lead to better performance. 
\begin{table}[h]
  \centering
...
\chapter{Discussion}
In this chapter, we will discuss our methodology, results from the experiments as well as the advantages and also the drawbacks of our method.

\textbf{Framework} \quad Our framework towards \gls{6dof} pose estimation is based on the diffusion pipeline and give this task a new way besides previous methods like direct regression and correspondance matching. We can share the advantages from the generative models especially the high diversity of the generation from the diffusion models. However this kind of framework also increase the complexity of training and real-world applications. We need to seperate the training phase and inference phase in the level of network structure and the inference is straightforward because the generation is actually iterative and muss go through a large number of steps. Although we have many optimization techniques on sampling that are introduced previously in section \ref{sec:impr_sample}, the inference time is still markable compared with many swift methods. The encoder networks which provide the diffusion backbone features from the input data are trained seperately besides diffuion networks. That means it is not an end-to-end model and we need some subtasks to pretrain the feature extractors or use the general pretrained models.

\textbf{Featue Fusion} \quad In our method, we utilize both \gls{2d} and \gls{3d} features from the input data to squeeze the information as much as possible. However the topic of how to fuse the feature in different domain is non-trival in the field of deep learning. Here we tried the most intuitive ways like addition and concatenation, but these kind of operations are not the most efficient and effective ones. From the ablation study (section \ref{sec:ab_fusion}) on this part, we can find that the way of feature fusion has a significant influence on the final result. 

Furthermore, we extract both the global features from the \gls{2d} and \gls{3d} domain to represent the object in a specific pose. The fusion of global feature is relative straightforward because we only need to match the feature dimension by using some projection layers. However, if we want to extract the local feature using patchified sampling or dense feature from keypoints, we need to further align the local feature from different domains which is also a non-trival task.

\textbf{2D Featue Extractor} \quad We use a self-supervised \gls{2d} network pretrained on general image dataset to extract the \gls{2d} feature instead of training on the target dataset. The reason is that we only need to encode the image with latent expression and a general pretrained model has the capability on the unseen \gls{rgb} data in the downstream task. From section \ref{sec:ab_feat} we may notice the poor performance when we only use \gls{2d} feature, especially under occlusion situations. Obviously, we can further improve the performance by training the \gls{2d} network using supervised learning on the pose estimation task to make the feature distribution more suitable in the corresponding manifold. But that not the purpose of our work that we want to show the effectiveness of the diffusion framework itself on processing fuzzy input features. Using a general network pretrained on a large number of images is a more flexible way to develop the diffusion models in different applications with different data, since we don't need to train the network every time from scratch.

\textbf{3D Featue Extractor} \quad In contrary to the \gls{2d} one, there is almost no general \gls{3d} feature extraction network that can handle unseen data with high accuracy for the downstream task. That is because the \gls{3d} data is much more sparse and the point cloud has the property of permutation invariance and only has geometrical information which causes more uncertainty in the feature extraction. To that end, we train our \gls{3d} network on the target dataset to extract the feature from a point cloud completion task. From section \ref{sec:ab_feat} we can validate our setting that \gls{3d} network itself can perform a quiet satisfied result. It is worth to mention that the network we use performs well on the single object but still have room for better generalization on multiple objects. If the occluded part exceed certain percentage of the whole object, it is hard for the network to complete the correct shape of the object without any prior knowledge.

\textbf{Diffusion Model} \quad We use the architecture of the \gls{ddpm} as our diffusion model and techniques from \gls{ddim} to accelerate the inference phase. From the visualization \ref{img:eval_vis_denoise}, we notice that no matter how we initialize the pose at first step, the pose hypotheses will converge to the ground truth pose after a certain number of denoising steps. The number of the denoising steps may have an effect of the quality of the generated sample in image synthesis task, but as in section \ref{sec:ab_steps} shows, it doesn't compose a markable difference in our pose estimation task. The reason may be that the denoising objective (transformation) is a low-dimensional representation of an \gls{3d} object compared with point cloud. \gls{2d} image it self is a high-dimensional matrix with more entities (pixels) and channels. We need more denoising steps to ensure the meaningful representaion and quality at pixel level and also image level of the generated sample. In our case, the transformation vector has less difficulty to converge to the ground truth pose and need less denoising steps.

Different from the orignial \gls{ddpm}, we use the Transformer encoder as our backbone to fit the form of input data instead of \gls{cnn} based network. Another advantage is we can make use of the self-attention mechanism to capture the weighted relationship between the input tokens. If we extend our work into pose estimation of the sequence input like video, or in other words, object tracking, the self-attention mechanism is able to understand the temporal relationship between the frames and make the estimation more robust, since the transformer encoder is originally designed for the sequence input.

In our experiment, we let the final output of generated pose to be the mean of multi-hypotheses, in order to further minimize the error with the ground truth and speed up the convergance of the model to some degree.  In the case of symetrical objects or the ambiguity caused by occlusion, the distribution of the pose hypotheses is no longer a single peak but a multi-modal distribution. When we estimate the pose of object with ambiguity, it is better to sample more hypotheses and analize the distribution of the pose, which is quiet useful in the application like robot grasping to find the best pose to grasp the object.
...
\section{Summary}
In this thesis, we present a novel framework and also the evaluation for \gls{6dof} pose estimation. \gls{6dof} pose estimation as an important task in the field of computer vision has been widly used in many applications like robotics and augmented reality. The common methods such as template-base, keypoint-based and direct regression have their own advantages and constrains in different scenarios. In our work, we propose the diffusion-based framework for \gls{6dof} pose estimation. 

Diffusion models are a class of generative models that can generate high-quality samples from a randomly initialized noise. We first review the classical diffusion models with some related improvement works. Then we design our diffusion model which is compatible with the pose estimation task. We use the Transformer encoder as our backbone to estimate the noise added at each time step of the training phase with the noisy pose and conditional guidance from the input data. In the reference phase, the pose is gradually denoised from the initial random noise to the final estimation by iteratively passing through the denoiser which is the transformer encoder. An optional multi-hypotheses and \gls{icp} refinement is applied to further improve the accuracy of the estimation.

To extract the feature from the input \gls{rgbd} data. A self-supervised \gls{vit} called DINO \cite{caron2021emerging} is utilized for downstreaming \gls{2d} feature and a point cloud completion task using FoldingNet \cite{yang2018foldingnet} is designed for \gls{3d} feature. After that, we fuse the feature from both domain to become the conditional embedding form the diffusion model.

To validate the performance of our method, we evaluate our method on \gls{lm} \cite{hinterstoisser2012model} dataset and the occlusion version \gls{lmo} \cite{dataV4MUMX2020} dataset with the comparison of other state-of-the-art methods. The result shows that our method achieves the comparable result with other methods even facing the objects with occlusion. Our attempts on the real-world dataset also shows the capability of diffusion-based framework on the pose estimation task. A well-rounded ablation study is also given to analyze the effect of different components in our method.

Our method as a pioneer work on the diffusion-based \gls{6dof} pose estiamtion task on the real-world dataset, shows a new approach to this task and also the potential of the diffusion models outside the field of image synthesis.

\section{Outlook}
There are still many aspects that can be further improved in the future research. In our thesis, we use the architecture of \gls{ddpm} as our diffusion model. The alternative model like \gls{sgm} \cite{song2020generative,song2020improved} can be also applied to our task to avoid the consideration of nomalizing constant in likelihood-based models. The sampling techniques like \gls{cas} \cite{jolicoeurmartineau2020adversarial}, \gls{ald} \cite{dockhorn2022scorebased} and \gls{ddss} \cite{watson2022learning} can be an alternative to the \gls{ddim} to accelerate the inference phase.

On the other hand, we can also invest on the feature extraction part. Besides using the general pretrained \gls{2d} network, we can train the network supervised with the target dataset to further improve the performance. For the \gls{3d} feature, technique like contrastive learning may be new direction to explore. The generalization of the \gls{3d} downstream network is the bottleneck of the flexiable usage of our method. If we can find a way to extract the \gls{3d} feature from the unseen data using a general model, our method can be applied to many other applications rapidly. The way of feature fusion is also a non-trival task. Besides the addition and concatination, we can also try the more advanced method like cross-attention to capture the relationship between the features from different domains.

Our method can be also extended to the task of object tracking. The transformer encoder as our diffusion backbone can be used to understand the relationship not only between the rotation and translation inside a single frame, but should also be able to learn the temporal relationship between the frames. And we can use the multi-hypotheses of diffusion models to generate the distribution of the pose instead of a single possible solution. It allow us to understand the ambiguity of the symetrical objects and find the best pose to track the object. 
...
\begin{figure}[H]
	...
	\caption{Rotation error under different representations of rotation during training phase (without refinement).}
	\label{img:ab_rot}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Translation error with and without using residual translation during training phase (without refinement).}
	\label{img:ab_trans}
\end{figure}
...
\begin{table}[H]
  \centering
  \caption{Comparison of model trained with and without residual translation on LM dataset}
...
\begin{figure}[H]
	...
	\caption{Visualization of some objects highlighted at estimated pose. Row 1-2: objects without occlusion; Row 3-4: objects with occlussion; Row 5: objects with occlussion shown in one scene}
	\label{img:vis_all}
\end{figure}
