...
Here we also demonstrate the qualitative result of our model on the pose estimation task. Figure \ref{img:eval_vis_img} shows some objects rendered with the estimated pose on the input image. First row is the objects without occlusion and second row is the objects with occlusion. We can find that the \gls{2d}-projection of the estimated pose has a very high persentage of overlap with the ground truth pose if there is no occlusion. The accuracy drops a little bit when the visible part of the object is occluded by other objects.
\begin{figure}[h]
	\centering
...
\textbf{Framework} \quad Our framework towards \gls{6dof} pose estimation is based on the diffusion pipeline and give this task a new way of solution besides direct regression and correspondance matching. We can share the advantages from the generative models especially the high diversity of the generation from the diffusion models. However this kind of framework also increase the complexity of training and real-world application. We need to seperate the training phase and inference phase in the level of network structure and the inference is straightforward because the generation is actually iterative and muss go through plenty of steps. Although we have many optimization on sampling phase that introduce previously in section \ref{sec:impr_sample}, the inference time is still markable compared with many swift methods. The encoder networks which provide the diffusion backbone features from the input data are trained seperately besides diffuion networks

\textbf{\gls{2d} Feature Extractor} \quad 
\chapter{Conclusion}
